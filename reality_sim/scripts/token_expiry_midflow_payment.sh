...
LOG="reality_sim/logs/token_expiry_midflow_payment_$(date +%Y%m%d_%H%M%S).log"
PSQL_URL="${DATABASE_URL%%\?*}"

echo "Starting token-expiry mid-flow (payment create)..." | tee -a "$LOG"

# provider
PROVEMAIL="prov_pay_$(date +%s%N)@test.local"
...
SLOTJSON=$(curl -s -X POST "$API/slots" \
  ...
  -d "{\"start\":\"$START_ISO\",\"end\":\"$END_ISO\",\"title\":\"Pay Slot\"}")
SLOT_ID=$(node -e 'const j=JSON.parse(process.argv[1]); console.log(j.slot.id);' "$SLOTJSON")

# patient
PATEMAIL="pat_pay_$(date +%s%N)@test.local"
...
BOOKJSON=$(curl -s -X POST "$API/appointments" \
  ...
  -d "{\"slotId\":$SLOT_ID,\"title\":\"Pay Test\"}")
...
# payment create with expired token
RESP=$(curl -s -i -X POST "$API/payments/v2" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $SHORT_TOKEN" \
  -d "{\"appointmentId\":$APPT_ID,\"amount\":1000,\"paymentType\":\"CARD\"}")

echo "Payment response (raw):" | tee -a "$LOG"
echo "$RESP" | tee -a "$LOG"
