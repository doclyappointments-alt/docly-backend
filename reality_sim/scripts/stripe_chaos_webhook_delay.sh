...
LOG="reality_sim/logs/stripe_chaos_webhook_delay_$(date +%Y%m%d_%H%M%S).log"
PSQL_URL="${DATABASE_URL%%\?*}"

echo "Starting Stripe Chaos — Webhook delay simulation (REAL TARGET)" | tee -a "$LOG"

# provider
PROVEMAIL="prov_stripe_$(date +%s%N)@test.local"
...
SLOTJSON=$(curl -s -X POST "$API/slots" \
  ...
  -d "{\"start\":\"$START_ISO\",\"end\":\"$END_ISO\",\"title\":\"Stripe Chaos Slot\"}")
SLOT_ID=$(node -e 'const j=JSON.parse(process.argv[1]); console.log(j.slot.id);' "$SLOTJSON")

# patient
PATEMAIL="pat_stripe_$(date +%s%N)@test.local"
...
BOOKJSON=$(curl -s -X POST "$API/appointments" \
  ...
  -d "{\"slotId\":$SLOT_ID,\"title\":\"Stripe Chaos Test\"}")
...
PAYLOAD=$(cat <<JSON
{
  "id": "evt_test_delay_$(date +%s)",
  "type": "charge.refunded",
  "data": {
    "object": {
      "id": "ch_test_delay",
      "amount_refunded": 1000,
      "payment_intent": "pi_test_delay",
      "metadata": {
...
echo "Injecting delayed SIGNED webhook (15s delay)..." | tee -a "$LOG"
sleep 15

RESP=$(curl -s -i -X POST "$API$WEBHOOK_PATH" \
  -H "Content-Type: application/json" \
  -H "Stripe-Signature: $SIG" \
  --data-binary "$PAYLOAD")

echo "Webhook response (raw):" | tee -a "$LOG"
echo "$RESP" | tee -a "$LOG"
